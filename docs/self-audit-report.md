...
| D-3 | Cerrada | **Resuelta (2026-08-19).** A-14 (declarar/publicar la URL del repositorio) se cerró: repositorio público, commit de referencia `98e9457` (rama `main`) declarado en el `.docx`. |
...
Se aplicaron y verificaron **todos** los hallazgos de este reporte (Crítico, Alto, Medio, Bajo/Nitpick, y D-1), no solo los recomendados como prioritarios en la Sección 6 original. El puntaje no llega a 10/10 porque: (a) D-2 (extensión de los anexos F-J) sigue abierto por decisión del usuario, no por trabajo pendiente de esta ronda; y (b) la verificación en vivo de COD-2 quedó parcialmente limitada por el alcance del laboratorio (ningún host de la whitelist permite fabricar el escenario "sin puerto HTTP" para observarlo correr de punta a punta), aunque sí se verificó exhaustivamente por inspección estática, reimportación exitosa en n8n real, y una corrida end-to-end real que ejercita la rama no afectada del mismo nodo.
...
| 9 | El nuevo validador de `target` (COD-5) cubre el flag-injection vía `target`, pero los valores de `options` (`ports` de Nmap, `severity`/`tags` de Nuclei, `max_time` de Nikto, `aggression` de WhatWeb) siguen sin validar y llegan igual de crudos al argv del subproceso. | **RESUELTO 2026-08-22** (ver Sección 9). Validador agregado a nivel del schema `ScanRequest.options` (mismo lugar que ya valida `target`), no en `scan_runner.execute()` — FastAPI convierte el `ValueError` del validador Pydantic en un 422 automáticamente, sin necesitar manejo adicional ahí. |
...
## 9. Cuarta ronda: plan post-evaluación independiente (2026-08-22)

Tras `docs/independent-evaluation-report.md` (7,5/10) y la decisión explícita de **no ejecutar el brazo manual** (riesgo de contaminación: el propio autor construyó los catálogos de verdad de referencia que mediría — ver conversación, no repetido acá), se armó un plan de mejora dirigido específicamente a lo que esa evaluación señaló (D2/D4/D5) y a los hallazgos de sus §2.1/2.2 que las 7 recomendaciones ya aplicadas (commits `b868276`..`98e9457`) no habían cubierto. Todo lo listado abajo fue verificado contra el código/documento reales antes de tocarse (no asumido desde memoria de sesiones previas).

**Documento** (`.docx`, backup implícito vía control de versiones de Word — TOC/footer verificados intactos tras cada guardado):
- §9.2 (marco epistemológico) reformulado para no reclamar más rigor del que un diseño n=5/un-target-principal sostiene — calibrado explícitamente contra §9.1/9.8/9.9 (D4 del evaluador independiente).
- §9.1 (diseño cuasi-experimental): la oración más larga partida en dos sin cambiar el contenido (D5).
- §9.9 (amenazas a la validez): agregado un párrafo con la réplica real de la campaña de medición contra `juice-shop` (ver abajo) — la amenaza "validez externa" pasa de solo declarada a parcialmente puesta a prueba.
- Esta misma sección (D-3, más arriba): corregida — decía que A-14 seguía pendiente, ya estaba cerrada desde el 2026-08-19.
- Se investigó si Amankwah et al. (2020, ya citado en la bibliografía) podía citarse como respaldo de literatura del hallazgo de 0% recall de alta confianza (§12.5) — **descartado**: el abstract real (verificado vía Crossref) dice que ZAP fue "efectivo", no reporta recall bajo. No se agregó ninguna cita basada en esto.

**Réplica de la campaña de medición contra `juice-shop`** (2026-08-22, `scripts/measurement_campaign_results/20260822T182945Z.json`/`_summary.md`): 5 corridas intentadas, 4 completadas + 1 fallo temprano legítimo (0,9 s, sin puerto HTTP detectado — el mismo camino de `Mark Scan Failed` de la Recomendación #2, activado de forma genuina). Orquestación: 0,63 s de media (CV 14,9%) — mismo orden de magnitud que los 0,85 s (CV 20,6%) contra `dvwa`. Tiempo total de pipeline más alto y variable (336,73 s, CV 14,9%, frente a 259,06 s/CV 4,1%) — Juice Shop (SPA Angular) es un objetivo más pesado, pero el componente de orquestación que automatiza el sistema se mantiene chico y estable en ambos targets. Nota: el propio script de captura de entorno (`docker` desde dentro del contenedor Backend) falló silenciosamente en esta corrida — el `_summary.md` de esta campaña no tiene versiones de herramientas capturadas, a diferencia del de `dvwa`; no se fabricó ese dato.

**Código, verificado abierto y corregido**:
- **Condición de carrera en `get_or_create_service`** (`backend/app/repositories/service_repository.py`): reescrito a insert-first + captura de `IntegrityError` dentro de un SAVEPOINT propio (mismo patrón que `target_service.py::register_target`), en vez de select-then-insert. Test nuevo agregado.
- **`Scan.pipeline_run_id` nunca se escribía** (documentado en `docs/database.md` como si correlacionara con la ejecución de n8n, pero ningún código lo hacía): ahora se puebla de verdad — los nodos `Complete Scan` y `Mark Scan Failed - Sin Puerto HTTP` de n8n mandan `$execution.id`, el Backend lo acepta y persiste. Verificado en vivo contra un scan real: `pipeline_run_id` quedó en 76/77 (los IDs de ejecución reales de n8n).
- **Ítem 9 de la Sección 8** (validación de `options` en los 4 adaptadores): cerrado, ver tabla arriba.
- **Escrituras de reporte no atómicas** (`reports/app/services/report_generator.py`): ahora escriben a un archivo temporal + `os.replace()` (helper `atomic_write` en `reports/app/paths.py`), en vez de escribir directo al nombre final. 4 tests nuevos.
- **Hardening de contenedores parcial**: `backend` pasó de solo `cap_drop` a también `read_only: true` + `tmpfs: /tmp` — se verificó primero (grep) que la API servida no escribe a disco en ningún punto; el único escritor real es `scripts/measurement_campaign.py` (herramienta de operador, no parte de la API servida), que ahora requiere desactivar `read_only` temporalmente para volver a correrse. `n8n`/`db` quedan sin tocar — necesitarían más volúmenes/tmpfs explícitos, verificación más profunda que no se hizo en esta ronda.

**Hallazgo nuevo, no corregido — encontrado durante la réplica de la campaña**: al ejecutar el segundo intento contra `juice-shop`, un `POST /scans/{id}/reports?format=pdf` sobre una corrida que sí había completado (no la que falló) devolvió `500 Internal Server Error` — `psycopg.errors.ForeignKeyViolation` sobre `reports_scan_id_fkey`. Causa: el nodo `Generate Report` del workflow de n8n corre *después* de que `Complete Scan` ya marcó el scan como `completed`; `scripts/measurement_campaign.py` sondea el estado, ve `completed`, y en su bloque `finally` borra el target (lo que en cascada borra el scan) antes de que `Generate Report` termine — carrera entre la limpieza del script y la cola de trabajo restante del propio pipeline. No es un bug introducido por esta ronda ni por la remediación anterior; es una condición preexistente recién expuesta por correr la campaña una segunda vez contra un target distinto. **No corregido** — no formaba parte del plan aprobado y el arreglo correcto (¿mover `Complete Scan` después de `Generate Report`? ¿que el script espere más antes de limpiar?) requiere decidir cuál de los dos comportamientos es el que se quiere, no es una corrección mecánica. Queda como observación abierta para una ronda futura.

**Verificación**: `backend` 122→125 tests, `scanner` 67→74 tests, `reports` 28→32 tests, todos pasan contra imágenes reconstruidas; workflow de n8n reimportado/reactivado; dos scans reales de punta a punta contra `dvwa` corridos después de los cambios (uno antes y otro después de activar `read_only` en `backend`), ambos completados normalmente con `pipeline_run_id` poblado.

**No incluido en esta ronda** (menor prioridad, ver conversación): cobertura de tests del frontend (sigue en 3 archivos); hardening de `n8n`/`db`.
